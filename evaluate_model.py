...
from model_v4_refined import RefinedCompliantModel, FORBIDDEN_COLUMNS
import warnings
...
def evaluate_model_cv(train_df, n_splits=5):
    ...
    # Group by race to ensure same race doesn't appear in train and val
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(race_ids, race_labels)):
        ...
        fold_results.append({
            ...
            'n_val_samples': len(fold_val),
            'n_val_races': len(val_races)
        })
...
if __name__ == "__main__":
    ...
    print(f"Unique races: {train_df['Race_ID'].nunique()}")
    print(f"Win rate: {(train_df['Position'] == 1).mean():.3%}")
    print()
