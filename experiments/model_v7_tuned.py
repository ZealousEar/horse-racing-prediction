...
class TunedModel:
    def __init__(self):
        ...
        # Keep V4 parameters but tune for Brier
    ...
    def create_specialization_features(self, df):
        ...
        df.rename(columns={'win_rate_smooth': 'trainer_course_win_rate', 'runs': 'trainer_course_experience'}, inplace=True)
        ...
        df.rename(columns={'win_rate_smooth': 'jockey_course_win_rate', 'runs': 'jockey_course_experience'}, inplace=True)
        ...
        df['trainer_jockey_course_synergy'] = df['trainer_course_win_rate'] * df['jockey_course_win_rate']
    ...
    def create_compliant_features(self, df):
        """Keep V4 features but REMOVE age categorical variables per feedback"""
        df = df.copy()
        ...
        # Age - KEEP AS CONTINUOUS ONLY (removed categorical per feedback)
        # Just use Age as-is, no young_horse, prime_age, veteran
    ...
    def train(self, train_df):
        ...
        # TUNED specifically for Brier score - using is_unbalance and scale_pos_weight
        n_pos = y.sum()
        n_neg = len(y) - n_pos
...
def main():
    print("V7 TUNED Model - Brier Score Optimized")
    print("="*50)
    print("Changes from V4:")
    print("- Removed age categorical features (per feedback)")
    print("- Hyperparameters tuned for Brier score")
    print("- Added class imbalance handling")
    print()
    ...
    predictions.to_csv('predictions_v7.csv', index=False)
